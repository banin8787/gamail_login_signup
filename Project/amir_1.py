...
def validate_signup():
    ...
    signup_window.destroy()  # بستن صفحه ثبت‌نام
    import signin  # وارد کردن ماژول signin
    signin.create_signin_page()  # باز کردن صفحه ورود
...
def validate_signin():
    ...
    # اعتبارسنجی اولیه برای مثال
    if username == "testuser" and password == "testpassword":
        messagebox.showinfo("Success", "Login successful!")
        signin_window.destroy()  # بستن صفحه ورود
        import signup  # وارد کردن ماژول signup
        signup.signup_window.deiconify()  # نمایش دوباره صفحه ثبت‌نام
    else:
...
def create_signin_page():
    ...
    passwordLabel = Label(Frame, text="Password", font=("Microsoft Yahei UI Light", 10, 'bold'), bg='white', fg='firebrick1')
    passwordLabel.grid(row=2, column=0, sticky='w', padx=25, pady=(10, 0))
    global passwordEntry
    passwordEntry = Entry(Frame, width=30, font=("Microsoft Yahei UI Light", 10, 'bold'), fg='black', bg='white', show='*')
    passwordEntry.grid(row=3, column=0, sticky='w', padx=25)
    
    signinButton = Button(Frame, text='Login', font=("Open Sans", 16, 'bold'), bd=0, bg='firebrick1', fg='white', activebackground='firebrick1', activeforeground='white', width=17, command=validate_signin)
    signinButton.grid(row=4, column=0, pady=11)
